...
$cotizaciones = CtrCotizaciones::listar();
$resumen = CtrCotizaciones::resumen();
...
<head>
    ...
    <title>Cotizaciones - TallerPro</title>
            ...
            <section class="mt-6 grid grid-cols-3 gap-2">
                ...
                <div class="rounded-[1.35rem] border border-white/10 bg-white/10 p-3 text-center">
                    <p class="text-xl font-black">
                        S/ <?php echo number_format((float)($resumen['monto_pendiente'] ?? 0), 0); ?>
                    </p>
            ...
            <form method="GET" class="soft-card rounded-[1.6rem] p-4 ring-1 ring-slate-200/70">
                ...
                <div class="mt-3 flex gap-2">
                    ...
                    <select
                        ...
                        <option value="CONVERTIDA" <?php echo $estadoActual === 'CONVERTIDA' ? 'selected' : ''; ?>>Convertidas</option>
                        <option value="RECHAZADA" <?php echo $estadoActual === 'RECHAZADA' ? 'selected' : ''; ?>>Rechazadas</option>
                    </select>
                    ...
                    <?php foreach ($cotizaciones as $cot): ?>
                        ...
                        <article class="soft-card rounded-[1.5rem] p-4 ring-1 ring-slate-200/70">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="mt-1 text-sm font-bold text-slate-500">
                                        <?php echo htmlspecialchars($cot['placa'] . ' · ' . $cot['marca'] . ' ' . $cot['modelo']); ?>
                                    </p>

                                    <p class="mt-1 text-xs font-bold text-slate-400">
                                        <?php echo date('d/m/Y H:i', strtotime($cot['created_at'])); ?>
                                    </p>
                            ...
                            <div class="mt-4 grid grid-cols-2 gap-2 border-t border-slate-100 pt-4">
                                ...
                                <?php if (!empty($cot['telefono'])): ?>
                                    <a
                                        href="https://wa.me/51<?php echo preg_replace('/[^0-9]/', '', $cot['telefono']); ?>?text=<?php echo urlencode('Hola, le comparto su presupuesto ' . $cot['codigo'] . ' por S/ ' . number_format((float)$cot['total'], 2)); ?>"
                                        target="_blank"
                ...
                <?php else: ?>
                    ...
                    <div class="rounded-[1.7rem] border border-dashed border-slate-300 bg-white/70 p-6 text-center">
                        ...
                        <p class="mx-auto mt-1 max-w-[260px] text-sm font-semibold text-slate-500">
                            Busca un vehículo por placa y genera un presupuesto desde su ficha.
                        </p>
