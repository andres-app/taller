...
$respuesta = CtrOrdenes::guardarDetallada();
$datos = CtrOrdenes::datosNueva();
...
<head>
    ...
    <title>Orden rápida - TallerPro</title>
...
<div class="app-shell mx-auto max-w-[430px] pb-40">
    ...
    <main class="px-5 pt-5">
        ...
        <form method="POST" id="formOrden">
            ...
            <input type="hidden" name="descripcion" id="descripcionOrden" value="Orden rápida">
            ...
            <section class="soft-card rounded-[1.8rem] p-5 ring-1 ring-slate-200/70">
                ...
                <p class="mt-1 text-sm font-semibold text-slate-500">
                    Toca una plantilla y luego ajusta productos, precios o pago.
                </p>

                <div class="mt-5 grid grid-cols-2 gap-3">

                    <button type="button" class="template-btn active" data-template="menor" onclick="seleccionarPlantilla('menor', this)">
                        <div class="text-3xl">🛢️</div>
                        <div class="mt-3 text-sm">Mantenimiento menor</div>
                    </button>

                    <button type="button" class="template-btn" data-template="mayor" onclick="seleccionarPlantilla('mayor', this)">
                        <div class="text-3xl">🔧</div>
                        <div class="mt-3 text-sm">Mantenimiento mayor</div>
                    </button>

                    <button type="button" class="template-btn" data-template="aceite" onclick="seleccionarPlantilla('aceite', this)">
                        <div class="text-3xl">💧</div>
                        <div class="mt-3 text-sm">Cambio de aceite</div>
                    </button>

                    <button type="button" class="template-btn" data-template="frenos" onclick="seleccionarPlantilla('frenos', this)">
                        <div class="text-3xl">🛑</div>
                        <div class="mt-3 text-sm">Revisión frenos</div>
                    </button>

                    <button type="button" class="template-btn" data-template="escaneo" onclick="seleccionarPlantilla('escaneo', this)">
                        <div class="text-3xl">🖥️</div>
                        <div class="mt-3 text-sm">Escaneo</div>
                    </button>

                    <button type="button" class="template-btn" data-template="lavado" onclick="seleccionarPlantilla('lavado', this)">
                        <div class="text-3xl">✨</div>
                        <div class="mt-3 text-sm">Lavado / detalle</div>
                    </button>
            ...
            <section class="mt-5 soft-card rounded-[1.8rem] p-5 ring-1 ring-slate-200/70">
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm font-semibold text-slate-500">
                            Stock descuenta inventario. Externo no descuenta.
                        </p>
                    </div>
                </div>

                <div id="itemsContainer" class="mt-5 space-y-4"></div>
            ...
            <section class="mt-5 soft-card rounded-[1.8rem] p-5 ring-1 ring-slate-200/70">
                ...
                <p class="mt-1 text-sm font-semibold text-slate-500">
                    Marca si pagó completo, quedó fiado o dejó adelanto.
                </p>

                <div class="mt-4">
                    <label class="field-label">Estado de pago</label>

                    <select name="pago_estado" id="pagoEstado" class="field" onchange="toggleAdelanto()">
                ...
                <div id="adelantoBox" class="mt-4 hidden">
                    <label class="field-label">Adelanto S/</label>
...
const categoriasMap = {
    aceite: ['ACEITE', 'LUBRICANTE', '20W', '15W', '10W'],
    filtro: ['FILTRO'],
    freno: ['FRENO', 'PASTILLA', 'ZAPATA', 'DISCO'],
};

const plantillas = {
    menor: {
        titulo: 'Mantenimiento menor',
        items: [
            { tipo: 'STOCK', categoria: 'aceite', descripcion: 'Aceite del motor' },
            { tipo: 'STOCK', categoria: 'filtro', descripcion: 'Filtro de aceite' },
            { tipo: 'SERVICIO', descripcion: 'Mano de obra mantenimiento menor', precio: 40 }
        ]
    },
    mayor: {
        titulo: 'Mantenimiento mayor',
        items: [
            { tipo: 'STOCK', categoria: 'aceite', descripcion: 'Aceite del motor' },
            { tipo: 'STOCK', categoria: 'filtro', descripcion: 'Filtro de aceite' },
            { tipo: 'STOCK', categoria: 'filtro', descripcion: 'Filtro de aire' },
            { tipo: 'SERVICIO', descripcion: 'Mano de obra mantenimiento mayor', precio: 90 }
        ]
    },
    aceite: {
        ...
        items: [
            { tipo: 'STOCK', categoria: 'aceite', descripcion: 'Aceite del motor' },
            { tipo: 'SERVICIO', descripcion: 'Mano de obra cambio de aceite', precio: 25 }
        ]
    },
    frenos: {
        titulo: 'Revisión de frenos',
        items: [
            { tipo: 'SERVICIO', descripcion: 'Revisión y diagnóstico de frenos', precio: 30 },
            { tipo: 'STOCK', categoria: 'freno', descripcion: 'Pastillas / repuesto de freno' }
        ]
    },
    escaneo: {
        titulo: 'Escaneo',
        items: [
            { tipo: 'SERVICIO', descripcion: 'Escaneo computarizado', precio: 40 }
        ]
    },
    lavado: {
        titulo: 'Lavado / detalle',
        items: [
            { tipo: 'SERVICIO', descripcion: 'Lavado o detalle vehicular', precio: 35 }
        ]
    }
};
...
function seleccionarPlantilla(tipo, btn) {
    document.querySelectorAll('.template-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');

    const plantilla = plantillas[tipo];

    document.getElementById('descripcionOrden').value = plantilla.titulo;

    limpiarItems();

    plantilla.items.forEach(item => {
        agregarItem(item.tipo, item);
    });

    calcularTotal();
}

function productoCoincideCategoria(producto, categoriaKey) {
    if (!categoriaKey) {
        return true;
    }

    const nombre = String(producto.nombre || '').toUpperCase();
    const categoria = String(producto.categoria || '').toUpperCase();
    const texto = nombre + ' ' + categoria;

    const palabras = categoriasMap[categoriaKey] || [];

    return palabras.some(p => texto.includes(p));
}

function agregarItem(tipo, preset = {}) {
    ...
    productos
        .filter(p => tipo !== 'STOCK' || productoCoincideCategoria(p, preset.categoria || ''))
        .forEach(p => {
            opcionesProductos += `
                <option 
                    value="${p.id}"
    ...
    const descripcion = preset.descripcion || '';
    const precio = preset.precio || 0;
    const costo = preset.costo || 0;
        ...
        <article class="rounded-[1.5rem] border border-slate-200 bg-white p-4 item-row">
            ...
            ${tipo === 'STOCK' ? `
                <div class="mb-3">
                    ...
                    <p class="mt-2 text-xs font-bold text-slate-500">
                        Si no está en stock, quita este item y agrega “Externo”.
                    </p>
                </div>
            ` : `
            ...
            <div class="mb-3">
                ...
                <input 
                    ...
                    value="${descripcion}"
                    placeholder="${tipo === 'SERVICIO' ? 'Ej. Mano de obra' : tipo === 'EXTERNO' ? 'Ej. Repuesto comprado afuera' : 'Producto'}"
                    ${tipo === 'STOCK' ? 'readonly' : ''}
                >
            </div>
...
function toggleAdelanto() {
    ...
    const box = document.getElementById('adelantoBox');
...
seleccionarPlantilla('menor', document.querySelector('[data-template="menor"]'));
</script>
