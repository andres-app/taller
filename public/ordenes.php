...
$ordenes = CtrOrdenes::listar();
$resumen = CtrOrdenes::resumen();
        ...
        <section class="mt-6 grid grid-cols-3 gap-2">
            ...
            <div class="rounded-[1.35rem] border border-white/10 bg-white/10 p-3 text-center">
                <p class="text-2xl font-black">
                    <?php echo (int)($resumen['total_ordenes'] ?? 0); ?>
                </p>
            ...
            <div class="rounded-[1.35rem] border border-white/10 bg-white/10 p-3 text-center">
                <p class="text-xl font-black text-red-200">
                    S/ <?php echo number_format((float)($resumen['total_fiado'] ?? 0), 0); ?>
                </p>
    ...
    <main class="px-5 pt-5">
        ...
        <form method="GET" class="soft-card rounded-[1.6rem] p-4 ring-1 ring-slate-200/70">
            ...
            <input
                ...
                placeholder="Cliente, placa, código..."
            ...
            <div class="mt-3 flex gap-2">
                ...
                >
                    ...
                    <option value="PENDIENTE" <?php echo $estadoActual === 'PENDIENTE' ? 'selected' : ''; ?>>Pendientes</option>
                    <option value="EN_PROCESO" <?php echo $estadoActual === 'EN_PROCESO' ? 'selected' : ''; ?>>En proceso</option>
                    <option value="FINALIZADO" <?php echo $estadoActual === 'FINALIZADO' ? 'selected' : ''; ?>>Finalizados</option>
                </select>
            ...
            <?php if (count($ordenes) > 0): ?>
                ...
                <?php foreach ($ordenes as $orden): ?>
                    ...
                    if ($orden['estado'] === 'FINALIZADO') {
                        ...
                        $estadoClass = 'bg-blue-50 text-blue-700';
                    }

                    $pagoClass = 'bg-emerald-50 text-emerald-700';
                    if ($orden['pago_estado'] === 'FIADO' || (float)$orden['saldo'] > 0) {
                        $pagoClass = 'bg-red-50 text-red-700';
                    } elseif ($orden['pago_estado'] === 'ADELANTO') {
                        $pagoClass = 'bg-amber-50 text-amber-700';
                    }
                    ?>
                    ...
                    <article class="soft-card rounded-[1.5rem] p-4 ring-1 ring-slate-200/70">
                        ...
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span class="rounded-full <?php echo $estadoClass; ?> px-3 py-1 text-[11px] font-black">
                                        <?php echo htmlspecialchars($orden['estado']); ?>
                                    </span>

                                    <span class="rounded-full <?php echo $pagoClass; ?> px-3 py-1 text-[11px] font-black">
                                        <?php echo htmlspecialchars($orden['pago_estado']); ?>
                                    </span>
                                </div>
                            ...
                            <div class="shrink-0 text-right">
                                ...
                                <?php if ((float)$orden['saldo'] > 0): ?>
                                    <p class="mt-1 text-xs font-black text-red-600">
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-2 border-t border-slate-100 pt-4">
                            ...
                            <?php if (!empty($orden['telefono'])): ?>
                                <a
                                    href="https://wa.me/51<?php echo preg_replace('/[^0-9]/', '', $orden['telefono']); ?>?text=<?php echo urlencode('Hola, su orden ' . ($orden['codigo'] ?: ('ORD-' . $orden['id'])) . ' está registrada por S/ ' . number_format((float)$orden['total'], 2)); ?>"
                                    target="_blank"
                                    class="rounded-2xl bg-emerald-500 px-4 py-3 text-center text-sm font-black text-white active:scale-95"
                                >
                                    WhatsApp
                                </a>
                            <?php else: ?>
                                <span class="rounded-2xl bg-slate-100 px-4 py-3 text-center text-sm font-black text-slate-400">
                                    Sin WhatsApp
                                </span>
                            <?php endif; ?>
            ...
            <?php else: ?>
                ...
                <div class="rounded-[1.7rem] border border-dashed border-slate-300 bg-white/70 p-6 text-center">
                    ...
                    <p class="mx-auto mt-1 max-w-[260px] text-sm font-semibold text-slate-500">
                        Busca un vehículo por placa y crea una orden rápida o detallada.
                    </p>
    ...
    <div class="fixed inset-x-0 bottom-0 z-50 bottom-safe">
        <nav class="bottom-bar relative rounded-[1.65rem] px-3 pb-2 pt-2 text-white">
            ...
            <div class="grid grid-cols-5 items-end text-center">
                ...
                <a href="ordenes.php" class="nav-item active">
